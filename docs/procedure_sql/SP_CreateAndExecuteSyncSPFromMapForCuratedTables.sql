CREATE DEFINER=`root`@`%` PROCEDURE `SP_CreateAndExecuteSyncSPFromMapForCuratedTables`(table_name varchar(100))
BEGIN
...
DESCRIPTION
Génère le code pour la SP de sync de données curated pour une table spécifique.
...
-Schemas de source doit etre curated
...
DECLARE SyncInfoProcessName VARCHAR ( 100 ) DEFAULT CONCAT('SP_CreateAndExecuteSyncSPFromMapForCuratedTables - ',table_name);
...
IF vs_source_table_schema <>'curated' THEN 
    SET @errmsg = CONCAT('ERROR: The schema of the source table ',vs_source_table_schema,': is not curated. Please use another SP to Create and Execute the Sync ');
        SIGNAL SQLSTATE '45000' SET
        MYSQL_ERRNO = errno,
        MESSAGE_TEXT = @errmsg;
END IF;
...
INSERT IGNORE INTO curated.`", vs_sync_tablename, "` -- SP_Sync_" ,table_name, CHAR(10),
" (
	", vs_list_columns_destination, "
)
SELECT 
	", vs_list_columns_source, "
FROM `", vs_source_table_schema,"`.`",vs_source_table_name,"` AS tbloriginal 
 ",IFNULL(CONCAT(' WHERE ',var_query_cond),' '),";
...
SET vs_result_str=CONCAT(vs_result_str,"
UPDATE curated.`",vs_sync_tablename,"` AS tbl_des -- SP_Sync_" ,table_name, CHAR(10),
