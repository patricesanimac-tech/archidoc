CREATE DEFINER=`root`@`%` PROCEDURE `SP_CreateAndExecuteSyncSPFromMapForRawD365Tables`(table_name varchar(100))
BEGIN
...
2025-12-18 Ajouter IFNULL à la vérification de IsDelete <> 'True'.
2025-11-04 Ajouter un index dans la table de base pour modifiedon
2025-11-04 Ezzeddine Chakroun [MERP-99] Commencer création.

DESCRIPTION
Génère le code pour la SP de sync de données d365raw à curated pour une table spécifique.
...
-Schemas de source doit etre d365_raw
...
DECLARE SyncInfoProcessName VARCHAR ( 100 ) DEFAULT CONCAT('SP_CreateAndExecuteSyncSPFromMapForRawD365Tables - ',table_name);
...
IF vs_source_table_schema ='d365_raw' THEN 
    SET vs_latest_version_column='versionnumber';
    SET vs_delete_column='IsDelete';
ELSE
    SET @errmsg = CONCAT('ERROR: The schema of the source table ',vs_source_table_schema,': is not d365_raw. Please use another SP to Create and Execute the Sync ');
        SIGNAL SQLSTATE '45000' SET
        MYSQL_ERRNO = errno,
        MESSAGE_TEXT = @errmsg;
END IF;
...
DECLARE sTimeResult TEXT DEFAULT '';
DECLARE errno SMALLINT UNSIGNED DEFAULT 10000;
DECLARE var_derniere_timestamp_base VARCHAR(25);
...
IF @pk_check > 0 THEN
  ...
  DROP PRIMARY KEY,
  ADD PRIMARY KEY (", vs_list_primary_keys,");
END IF;

-- Déterminer le dernier timestamp de modification
SELECT  -- SP_Sync_", table_name, "
 DATE_FORMAT(DATE_SUB(
  IFNULL(
    MAX(STR_TO_DATE(REPLACE(REPLACE(modifiedon, 'T', ' '), 'Z', ''), '%Y-%m-%d %H:%i:%s')),
    '2000-01-01 00:00:00'
  ),
  INTERVAL 3 MINUTE
), '%Y-%m-%dT%H:%i:%sZ')
...
CREATE OR REPLACE TEMPORARY TABLE `", vs_source_table_schema,"`.`tbltmp_max_vn_by_id_",vs_source_table_name,"` -- SP_Sync_", table_name, CHAR(10), " (
...
WHERE  modifiedon >= var_derniere_timestamp_base
...
WHERE IFNULL(",vs_delete_column,", '') <> 'True'",IFNULL(CONCAT(' AND ',var_query_cond),' '),";
